...
───┴───────────────────────
</code></pre></div><p>这是一个很大的进步，从 30 秒到 2 秒。干得好，Pandas!</p><p>也许我们加载数据可以再快一点，这一次我们将使用 Nushell 的 <code>dfr open</code> 命令：</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code><span class="token operator">&gt;</span> benchmark <span class="token punctuation">{</span>dfr <span class="token function">open</span> .<span class="token punctuation">\\</span>Data7602DescendingYearOrder.csv<span class="token punctuation">}</span>
...
</code></pre></div><p>保存该文件并随意命名，在这些例子中，该文件将被称为 <code>test_small.csv</code>。</p><p>现在，要将该文件作为 DataFrame 进行读取，请使用 <code>dfr open</code> 命令，如下所示：</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code><span class="token operator">&gt;</span> <span class="token builtin class-name">let</span> <span class="token function">df</span> <span class="token operator">=</span> <span class="token punctuation">(</span>dfr <span class="token function">open</span> test_small.csv<span class="token punctuation">)</span>
</code></pre></div><p>这应该会在内存中创建一个值 <code>$df</code>，用来存放我们刚刚创建的数据。</p><div class="custom-container tip"><p class="custom-container-title">TIP</p><p><code>dfr open</code> 命令可以读取 <strong>csv</strong> 或 <strong>parquet</strong> 文件。</p></div><p>要查看存储在内存中的所有 DataFrames，你可以使用：</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code><span class="token operator">&gt;</span> dfr <span class="token function">ls</span>
...
</code></pre></div><div class="custom-container tip"><p class="custom-container-title">TIP</p><p>输入 <code>let res = ( !! )</code> 并按回车，这将自动完成之前执行的命令。注意 ( 和 !! 之间的空格。</p></div><p>现在我们有两个 DataFrame 存储在内存中：</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code><span class="token operator">&gt;</span> dfr <span class="token function">ls</span>
...
7,12,0.2,1.0,a
6,11,0.1,0.0,b
</code></pre></div><p>我们使用 <code>dfr open</code> 命令来创建新的变量：</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code><span class="token operator">&gt;</span> <span class="token builtin class-name">let</span> df_a <span class="token operator">=</span> <span class="token punctuation">(</span>dfr <span class="token function">open</span> test_small_a.csv<span class="token punctuation">)</span>
...
───┴───┴───┴────┴────
</code></pre></div><p>Nushell 强大的管道语法允许我们通过从其他 DataFrame 中获取数据并将其附加到这些 DataFrame 中来创建新的 DataFrame。现在，如果你列出你的 DataFrame，你会看到总共有四个：</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code><span class="token operator">&gt;</span> dfr <span class="token function">ls</span>
