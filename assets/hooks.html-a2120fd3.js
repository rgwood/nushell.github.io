import{_ as c,M as r,p as d,q as l,Q as o,t as e,N as n,U as i,a1 as t}from"./framework-344bb0e4.js";const h={},u=o("h1",{id:"hooks",tabindex:"-1"},[o("a",{class:"header-anchor",href:"#hooks","aria-hidden":"true"},"#"),e(" Hooks")],-1),p={href:"https://en.wikipedia.org/wiki/Read%E2%80%93eval%E2%80%93print_loop",target:"_blank",rel:"noopener noreferrer"},g=o("code",null,"nu script.nu",-1),f=o("code",null,'nu -c "print foo"',-1),m=t('<p>Currently, we support these types of hooks:</p><ul><li><code>pre_prompt</code> : Triggered before the prompt is drawn</li><li><code>pre_execution</code> : Triggered before the line input starts executing</li><li><code>env_change</code> : Triggered when an environment variable changes</li><li><code>display_output</code> : A block that the output is passed to (experimental).</li><li><code>command_not_found</code> : Triggered when a command is not found.</li></ul><p>To make it clearer, we can break down Nushell&#39;s execution cycle. The steps to evaluate one line in the REPL mode are as follows:</p><ol><li>Check for <code>pre_prompt</code> hooks and run them</li><li>Check for <code>env_change</code> hooks and run them</li><li>Display prompt and wait for user input</li><li>After user typed something and pressed &quot;Enter&quot;: Check for <code>pre_execution</code> hooks and run them</li><li>Parse and evaluate user input</li><li>If a command is not found: Run the <code>command_not_found</code> hook. If it returns a string, show it.</li><li>If <code>display_output</code> is defined, use it to print command output</li><li>Return to 1.</li></ol><h2 id="basic-hooks" tabindex="-1"><a class="header-anchor" href="#basic-hooks" aria-hidden="true">#</a> Basic Hooks</h2>',5),v=t(`<div class="language-text" data-ext="text"><pre class="language-text"><code>let-env config = {
    # ...other config...
...
</code></pre></div><h2 id="changing-environment" tabindex="-1"><a class="header-anchor" href="#changing-environment" aria-hidden="true">#</a> Changing Environment</h2>`,7),k=o("strong",null,"block",-1),x=o("code",null,"def-env",-1),_=t(`<div class="language-text" data-ext="text"><pre class="language-text"><code>&gt; let-env config = ($env.config | upsert hooks {
...
</code></pre></div>`,5),b=o("code",null,"if",-1),y=o("code",null,"load-env (if $after == ... { ... } else { {} })",-1),w=t(`<p>To deal with the above problem, we introduce another way to define a hook -- <strong>a record</strong>:</p><div class="language-text" data-ext="text"><pre class="language-text"><code>let-env config = ($env.config | upsert hooks {
    env_change: {
        PWD: [
            {
...
</code></pre></div>`,22),$=o("code",null,"file:///tmp/nu-output.html",-1),q=o("code",null,"save",-1),P=t(`<h3 id="command-not-found-hook-in-arch-linux" tabindex="-1"><a class="header-anchor" href="#command-not-found-hook-in-arch-linux" aria-hidden="true">#</a> <code>command_not_found</code> hook in <em>Arch Linux</em></h3><p>The following hook uses the <code>pkgfile</code> command, to find which packages commands belong to in <em>Arch Linux</em>.</p><div class="language-text" data-ext="text"><pre class="language-text"><code>let-env config = {
    ...other config...
...
</code></pre></div>`,3);function T(W,A){const s=r("ExternalLinkIcon"),a=r("RouterLink");return d(),l("div",null,[u,o("p",null,[e("Hooks allow you to run a code snippet at some predefined situations. They are only available in the interactive mode ("),o("a",p,[e("REPL"),n(s)]),e("), they do not work if you run a Nushell with a script ("),g,e(") or commands ("),f,e(") arguments.")]),m,o("p",null,[e("To enable hooks, define them in your "),n(a,{to:"/book/configuration.html"},{default:i(()=>[e("config")]),_:1}),e(":")]),v,o("p",null,[e("One feature of the hooks is that they preserve the environment. Environment variables defined inside the hook "),k,e(" will be preserved in a similar way as "),n(a,{to:"/book/environment.html#defining-environment-from-custom-commands"},{default:i(()=>[x]),_:1}),e(". You can test it with the following example:")]),_,o("p",null,[e("This won't work because the environment will be active only within the "),n(a,{to:"/commands/docs/if.html"},{default:i(()=>[b]),_:1}),e(" block. In this case, you could easily rewrite it as "),y,e(" but this pattern is fairly common and later we'll see that not all cases can be rewritten like this.")]),w,o("p",null,[e("You can view the result by opening "),$,e(" in a web browser. Of course this isn't very convenient unless you use a browser that automatically reloads when the file changes. Instead of the "),n(a,{to:"/commands/docs/save.html"},{default:i(()=>[q]),_:1}),e(" command, you would normally customize this to send the HTML output to a desired window.")]),P])}const E=c(h,[["render",T],["__file","hooks.html.vue"]]);export{E as default};
