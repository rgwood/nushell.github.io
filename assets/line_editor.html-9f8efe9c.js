...
let-env PROMPT_MULTILINE_INDICATOR <span class="token operator">=</span> <span class="token string">&quot;::: &quot;</span>
</code></pre></div><div class="custom-container tip"><p class="custom-container-title">TIP</p><p>提示符是环境变量，它代表了提示的状态</p></div><h2 id="按键绑定" tabindex="-1"><a class="header-anchor" href="#按键绑定" aria-hidden="true">#</a> 按键绑定</h2><p>Reedline 按键绑定是一个强大的结构，它允许你建立一连串的事件，而且这些事件可以通过特定的按键组合来触发。</p><p>例如，我们假设你想把补全菜单绑定到 <code>Ctrl + t</code> 这组快捷键上（默认是<code>tab</code>）。你可以添加下面的条目到你的配置文件：</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code>  let-env config <span class="token operator">=</span> <span class="token punctuation">{</span>
    <span class="token punctuation">..</span>.
  ...
  <span class="token punctuation">]</span>
  <span class="token punctuation">..</span>.
</code></pre></div><p>上述第一个按键绑定例子遵循第一种情况，只有一个事件被发送到引擎。</p><p>后一个按键绑定的例子是向引擎发送一系列的事件。它首先清除提示，插入一个字符串，然后输入该值。</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code>  let-env config <span class="token operator">=</span> <span class="token punctuation">{</span>
    <span class="token punctuation">..</span>.
  ...
  <span class="token punctuation">}</span>
    ...
    <span class="token punctuation">..</span>.
  <span class="token punctuation">}</span>
</code></pre></div><h3 id="移除一个默认的按键绑定" tabindex="-1"><a class="header-anchor" href="#移除一个默认的按键绑定" aria-hidden="true">#</a> 移除一个默认的按键绑定</h3><p>如果你想删除某个默认的按键绑定，而不打算使用不同的动作来替代它，你可以设置<code>event: null</code>。</p><p>例如，在所有的编辑模式下，禁用 <code>Ctrl + l</code> 清除屏幕：</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code>  let-env config <span class="token operator">=</span> <span class="token punctuation">{</span>
    <span class="token punctuation">..</span>.
  ...
  <span class="token punctuation">}</span>
    ...
    <span class="token punctuation">]</span>
    <span class="token punctuation">..</span>.
</code></pre></div><h3 id="补全菜单" tabindex="-1"><a class="header-anchor" href="#补全菜单" aria-hidden="true">#</a> 补全菜单</h3><p>补全菜单是一个上下文敏感的菜单，它将根据提示的状态给出建议。这些建议的范围包括从路径建议到替代命令。在编写命令时，你可以激活该菜单以查看内部命令的可用选项。另外，如果你已经为外部命令定义了你的自定义补全方式，这些补全提示也会出现在菜单中。</p><p>默认情况下，补全菜单是通过按<code>tab</code>访问的，它可以通过修改配置对象中的这些值来进行配置：</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code>  let-env config <span class="token operator">=</span> <span class="token punctuation">{</span>
    <span class="token punctuation">..</span>.
...
<span class="token punctuation">}</span>
    ...
    <span class="token punctuation">..</span>.
  <span class="token punctuation">}</span>
</code></pre></div>`,126);function k(h,m){const s=e("ExternalLinkIcon");return p(),o("div",null,[i,a("p",null,[n("Nushell 的行编辑器 "),a("a",u,[n("Reedline"),c(s)]),n(" 是一个跨平台的行读取器，它被设计为模块化且颇具灵活性的。该引擎的作用是负责控制命令历史，验证，自动补全，提示以及屏幕绘制。")]),r])}const b=t(l,[["render",k],["__file","line_editor.html.vue"]]);export{b as default};
